...
LOCAL_DIR=/data/vision/oliva/scratch/datasets/BOLDMomentsDataset
...
for f in "${dataset_files[@]}"; do
    aws s3 cp --no-sign-request s3://openneuro.org/ds4XXX488/${f} \
    ${LOCAL_DIR}/Nifti/
done

for sub in {01..10}; do
    mkdir ${LOCAL_DIR}/Nifti/sub-${sub}
    aws s3 sync --no-sign-request s3://openneuro.org/ds0XXX488/sub-${sub}/ \
